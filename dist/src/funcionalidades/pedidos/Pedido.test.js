...
const Produto_1 = require("@src/funcionalidades/produtos/Produto");
describe('Pedido', () => {
    const produto1 = new Produto_1.Produto('Produto A', 'Descrição A', 10.00, [{ sku: 'PA1', preco: 10.00, estoque: 100, atributos: {} }]);
    const produto2 = new Produto_1.Produto('Produto B', 'Descrição B', 20.00, [{ sku: 'PB1', preco: 20.00, estoque: 50, atributos: {} }]);
    it('deve criar uma instância de Pedido', () => {
        const pedido = new Pedido_1.Pedido('cliente123', [
            { produto: produto1, quantidade: 2, precoUnitario: 10.00 }
        ]);
        expect(pedido).toBeInstanceOf(Pedido_1.Pedido);
    });
    ...
    it('não deve validar um item com quantidade zero ou negativa', () => {
        ...
        expect(pedido.validar()).toBe(false);
        const pedidoNegativo = new Pedido_1.Pedido('cliente123', [
            { produto: produto1, quantidade: -1, precoUnitario: 10.00 }
        ]);
