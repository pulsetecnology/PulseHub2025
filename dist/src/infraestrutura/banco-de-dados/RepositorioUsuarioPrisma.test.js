...
const RepositorioUsuarioPrisma_1 = require("./RepositorioUsuarioPrisma");
...
describe('RepositorioUsuarioPrisma', () => {
    let repository;
    beforeEach(() => {
        repository = new RepositorioUsuarioPrisma_1.RepositorioUsuarioPrisma();
        jest.clearAllMocks();
    });
    it('deve chamar findByEmail no modelo Prisma', async () => {
        ...
        prismaClient_1.default.usuario.findUnique.mockResolvedValue(mockUser);
        const result = await repository.findByEmail('teste@example.com');
        expect(prismaClient_1.default.usuario.findUnique).toHaveBeenCalledWith({ where: { email: 'teste@example.com' } });
        expect(result).toEqual(mockUser);
    });
    it('deve retornar null se o usuário não for encontrado por email', async () => {
        prismaClient_1.default.usuario.findUnique.mockResolvedValue(null);
        const result = await repository.findByEmail('naoexiste@example.com');
        expect(prismaClient_1.default.usuario.findUnique).toHaveBeenCalledWith({ where: { email: 'naoexiste@example.com' } });
    ...
    it('deve chamar findAll ao buscar todos', async () => {
        ...
        prismaClient_1.default.usuario.findMany.mockResolvedValue(mockUsers);
    ...
    it('deve chamar create ao criar um item', async () => {
        ...
        prismaClient_1.default.usuario.create.mockResolvedValue(createdUser);
        const result = await repository.create(newUser);
        expect(prismaClient_1.default.usuario.create).toHaveBeenCalledWith({ data: newUser });
    ...
    it('deve chamar update ao atualizar um item', async () => {
        ...
        prismaClient_1.default.usuario.update.mockResolvedValue(updatedUser);
        const result = await repository.update('1', updatedData);
        expect(prismaClient_1.default.usuario.update).toHaveBeenCalledWith({ where: { id: '1' }, data: updatedData });
...
//# sourceMappingURL=RepositorioUsuarioPrisma.test.js.map