...
describe('RepositorioUsuarioPrisma', () => {
  ...
  it('deve chamar findByEmail no modelo Prisma', async () => {
    ...
    expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { email: 'teste@example.com' } });
  ...
  it('deve retornar null se o usuário não for encontrado por email', async () => {
    ...
    expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { email: 'naoexiste@example.com' } });
  ...
  it('deve chamar update ao atualizar um item', async () => {
    ...
    expect(prisma.user.update).toHaveBeenCalledWith({ where: { id: '1' }, data: updatedData });
